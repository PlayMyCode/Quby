...
module parse {
    ...
    interface MaybeTerm {
        ...
        ( match:string   ): Term;
        ( match:number   ): Term;
        ( match:TerminalFunction ): Term;
        ( match:MaybeTerm[]  ): Term;
    }

    interface MatchFoundFunction {
        ( ...args: any[] ): any;
    }
    ...
    class SymbolError extends ParserError {
        isSymbol = true;
    ...
    class TerminalError extends ParserError {
        isTerminal = true;
    ...
    interface DebugCallback { ( terms: Term[], times: TimeResult ) : void ; }

    interface SymbolizeCallback { ( terminals: Term[], errors: SymbolError[] ): void; } ;

    interface FinishCallback { ( result:any, errors:ParserError[] ): void; };
    ...
    var addRule: {
        ...
        function( rule, terminals:Term[], id:number, allRules:ParserRule[] ) {
            if ( rule instanceof Term ) {
                ...
                return (<ParserRule>rule).optimizeScan( terminals, id, allRules );
            }
        };

    var bruteScan: { ( parserRule: ParserRule, seenRules: bool[], idsFound: bool[] ): void; } =
        function( parserRule:ParserRule, seenRules:bool[], idsFound:bool[] ) {
        ...
        */
        ...
        seperatingRule( match, seperator ) {
            ...
            return this.thenSingle(
                    this.parseParent.parse().markSeperatingRule( match, seperator )
            );
        }

        markSeperatingRule( match, seperator ) {
            this.
                    thenAll( match ).
                    thenAll( seperator ).
                    endCurrentOr();
        ...
        thenAll( obj: IArguments ) {
            return this.endCurrentOr().helperAll( 'thenSingle', obj );
        }

        helperAll( singleMethod:string, obj: IArguments ) : ParserRule {
            this.errorIfEnded();
        ...
        parseInner( input:string, parseInput:string, callback:FinishCallback, debugCallback?:DebugCallback, name?:string ) : void {
        ...
        symbolizeInner( input: string, parseInput: string, callback: SymbolizeCallback ) : void {
         ...
         */
        parseSymbols( input: string, parseInput: string, callback: { ( symbols: SymbolResult, time: number ): void; } ) : void {
        ...
        parseRules( symbols:SymbolResult, inputSrc:string, src:string ) {
        ...
        ruleTest( symbols:SymbolResult, inputSrc:string ) : ()=>any {
        ...
        ruleTestSeperator( symbols:SymbolResult, inputSrc:string ) : ()=>any {
        ...
        ruleTestNormal( symbols: SymbolResult, inputSrc: string ): { (): any; }[] {
            /*
        ...
        ruleTestCyclic( symbols: SymbolResult, inputSrc: string ): { (): any; }[] {
            var args = null,
         ...
         */
        parseSymbolsInner( inputSrc: string, src: string, name: string ) {
    ...
    */
    ...
    var ignoreSingle: {
            ...
            ( ps:Parse, match:MaybeTerm[] ): void;
    } =
        function( ps:Parse, terminal ) {
            if ( terminal instanceof String || isFunction( terminal ) ) {
                ignoreSingle( ps, parse.terminal(terminal) );
            } else if ( terminal instanceof Term ) {
                ingoreInner( ps, terminal );
            } else if ( terminal instanceof Array ) {
                for ( var i = 0; i < terminal.length; i++ ) {
                    ignoreSingle( ps, terminalsInner( terminal[i], null ) );
                }
            } else if ( terminal instanceof Object ) {
                for ( var k in terminal ) {
                    if ( terminal.hasOwnProperty( k ) ) {
                        ignoreSingle( ps, terminalsInner( terminal[k], k ) );
                    }
                }
            } else {
    ...
    */
    ...
    var newParserRule = function( ps: Parse, args: IArguments ): ParserRule {
        if ( arguments.length === 0 ) {
            return new ParserRule( this );
        } else {
    ...
    export class Parse {
        ...
        parse() {
            return newParserRule( this, arguments );
        }
        ...
        or() {
            return new ParserRule( this ).orAll( arguments );
        }
        either() {
            return new ParserRule( this ).orAll( arguments );
        }
        ...
        a() {
            return new ParserRule( this ).thenAll( arguments );
        }
        then() {
            return new ParserRule( this ).thenAll( arguments );
        }
        ...
        optional() {
            return new ParserRule( this ).optionalAll( arguments );
        }
        maybe() {
            return new ParserRule( this ).optionalAll( arguments );
        }
         ...
         */
        repeatEither() {
            return new ParserRule( this ).cyclicOr( arguments );
        }
    ...
    export function parse() {
        return newParserRule( pInstance, arguments );
    }

    export function a() {
        return new ParserRule( pInstance ).thenAll( arguments );
    }
    ...
    export function or() {
        return new ParserRule( pInstance ).orAll( arguments );
    }
    ...
    export function optional() {
        return new ParserRule( pInstance ).optionalAll( arguments );
    }
    ...
    export function repeatSeperator( match, seperator ) {
        return new ParserRule( pInstance ).repeatSeperator( match, seperator );
    }

    export function optionalSeperator( match, seperator ) {
        return new ParserRule( pInstance ).optionalSeperator( match, seperator );
    }

    export function repeatEither() {
        return new ParserRule( pInstance ).cyclicOr( arguments );
    }
    ...
    export var terminal = <{
        ...
        var terminal:any = function( match:any, termName?:string ) : Term {
            if ( match instanceof Term ) {
                ...
                return new Term( match, termName ).setID( this.terminalID++ );
            }
        }
